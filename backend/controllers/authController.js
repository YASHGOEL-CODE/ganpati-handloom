...
const register = async (req, res) => {
  try {
    ...
    // ── CHANGED: Create user with isVerified: true — no email verification needed ──
    const user = await User.create({
      fullName,
      email,
      password,
    ...
    console.log('✅ User created (verified):', user._id);
...
// @desc    Verify email — kept for backward compatibility but not used in signup flow
// @route   GET /api/auth/verify-email/:token
// @access  Public
const verifyEmail = async (req, res) => {
  try {
...
// @desc    Resend verification email — kept for backward compatibility
// @route   POST /api/auth/resend-verification
// @access  Public
const resendVerification = async (req, res) => {
  try {
...
const login = async (req, res) => {
  try {
    ...
    // ── REMOVED: email verification check — all users are verified on registration ──
