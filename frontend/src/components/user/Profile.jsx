...
// import { userAPI } from '../../services/api'; // add if not already imported
...
//                               style={{ fontSize:12, color:'rgba(77, 6, 6, 0.38)', textDecoration:'none', marginBottom:4 }}
//                             >
//                               View Details →
//                             </Link>
//                             <button
...
import { useAuth } from '../../hooks/useAuth';
import { userAPI, addressAPI, ordersAPI } from '../../services/api';
import { CartContext } from '../../context/CartContext';
import { WishlistContext } from '../../context/WishlistContext';
import { formatPrice, formatDate } from '../../utils/helpers';
import { getImageUrl } from '../../utils/imageHelper';
...
import {
  ...
  FiMapPin, FiTrash2, FiCheck, FiPlus, FiX,
  FiPackage, FiHeart, FiLock, FiLogOut, FiAlertTriangle,
  FiShield, FiChevronRight, FiClock, FiTruck, FiShoppingCart,
} from 'react-icons/fi';
import { isValidEmail, isValidPhone } from '../../utils/helpers';
  ...
  return (
    <>
      <style>{`
        ...
        .pr-header { padding: 28px 0 22px; margin-bottom: 20px; }
        .pr-header-card {
          background: rgba(255,255,255,0.04); backdrop-filter: blur(20px);
          border: 1px solid rgba(255,255,255,0.08); border-radius: 20px;
          padding: 22px 28px; margin-bottom: 18px; position: relative;
        ...
        .pr-hero-card {
          background: rgba(255,255,255,0.04); backdrop-filter: blur(20px);
          border: 1px solid rgba(255,255,255,0.08); border-radius: 22px; padding: 28px 28px;
        ...
        .pr-avatar-ring {
          width: 88px; height: 88px; border-radius: 50%; flex-shrink: 0;
          padding: 4px; background: linear-gradient(135deg, #ea580c, #c084fc);
          position: relative; z-index: 1;
        }
        .pr-avatar-inner {
          width: 100%; height: 100%; border-radius: 50%;
        ...
        .pr-hero-badge {
          display: inline-flex; align-items: center; gap: 5px;
          background: rgba(74,222,128,0.12); border: 1px solid rgba(74,222,128,0.22);
          color: #4ade80; font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 999px;
        }
        .pr-stats {
          display: flex; position: relative; z-index: 1;
          border-left: 1px solid rgba(255,255,255,0.07);
          padding-left: 24px; flex-wrap: wrap; gap: 20px;
        }
        @media (max-width: 640px) { .pr-stats { border-left: none; padding-left: 0; border-top: 1px solid rgba(255,255,255,0.07); padding-top: 16px; width: 100%; } }
        .pr-stat { text-align: center; }
        .pr-stat-val {
        ...
        .pr-tab {
          ...
          border: 1.5px solid rgba(255,255,255,0.09);
          background: rgba(255,255,255,0.04);
          color: rgba(255,255,255,0.45); cursor: pointer; transition: all .22s ease;
        }
        .pr-tab:hover:not(.active) { border-color: rgba(255,255,255,0.18); color: rgba(255,255,255,0.80); background: rgba(255,255,255,0.07); transform: translateY(-1px); }
        .pr-tab.active { background: linear-gradient(135deg, #ea580c, #f97316); border-color: transparent; color: #fff; box-shadow: 0 4px 16px rgba(234,88,12,0.35); transform: translateY(-1px); }
        .pr-tab-ico { flex-shrink: 0; }

        .pr-card {
          background: rgba(255,255,255,0.04); backdrop-filter: blur(20px);
          border: 1px solid rgba(255,255,255,0.08); border-radius: 22px; padding: 32px;
          box-shadow: 0 12px 40px rgba(0,0,0,0.28), inset 0 1px 0 rgba(255,255,255,0.06);
          position: relative; overflow: hidden;
        }
        .pr-card::before {
        ...
        .pr-sec-title {
          ...
          flex-wrap: wrap; gap: 12px; padding-bottom: 14px; border-bottom: 1px solid rgba(255,255,255,0.06);
        }
        .pr-edit-btn {
          display: inline-flex; align-items: center; gap: 6px;
          font-size: 13px; font-weight: 600; color: #fb923c;
        ...
        .pr-success {
          ...
          background: rgba(74,222,128,0.10); border: 1px solid rgba(74,222,128,0.22);
          border-radius: 12px; padding: 13px 16px; margin-bottom: 22px;
          font-size: 13.5px; font-weight: 500; color: #4ade80; animation: prFadeIn .35s ease;
        }
        @keyframes prFadeIn { from{opacity:0;transform:translateY(-6px)} to{opacity:1;transform:translateY(0)} }
        .pr-error-banner {
          background: rgba(239,68,68,0.10); border: 1px solid rgba(239,68,68,0.22);
          border-radius: 12px; padding: 12px 16px; margin-bottom: 18px; font-size: 13.5px; color: #fca5a5;
        }
        .pr-field-err { font-size: 12px; color: #f87171; margin-top: 5px; }
        .pr-field { margin-bottom: 20px; }
        .pr-label {
          display: flex; align-items: center; gap: 6px; font-size: 11.5px; font-weight: 700;
          color: rgba(255,255,255,0.38); text-transform: uppercase; letter-spacing: .09em; margin-bottom: 8px;
        }
        .pr-input {
          width: 100%; background: rgba(0,0,0,0.28); border: 1.5px solid rgba(255,255,255,0.09);
          border-radius: 12px; padding: 13px 16px; color: #f1f5f9; font-size: 14.5px; font-weight: 500;
          font-family: 'DM Sans', sans-serif; outline: none;
          transition: border-color .22s, background .22s, box-shadow .22s;
        }
        .pr-input::placeholder { color: rgba(255,255,255,0.22); }
        .pr-input:disabled { opacity: .55; cursor: not-allowed; }
        .pr-input:focus { border-color: #ea580c; background: rgba(234,88,12,0.06); box-shadow: 0 0 0 3px rgba(234,88,12,0.14), 0 2px 12px rgba(234,88,12,0.10); }
        .pr-input:hover:not(:disabled):not(:focus) { border-color: rgba(255,255,255,0.18); }
        .pr-btn-row { display: flex; gap: 12px; margin-top: 6px; }
        .pr-save-btn {
          flex: 1; display: flex; align-items: center; justify-content: center; gap: 8px;
          background: linear-gradient(135deg, #ea580c, #f97316); color: #fff; border: none;
          border-radius: 12px; padding: 13px; font-size: 14px; font-weight: 700;
          font-family: 'DM Sans', sans-serif; cursor: pointer; transition: transform .22s, box-shadow .22s;
          box-shadow: 0 4px 16px rgba(234,88,12,0.28);
        }
        .pr-save-btn:hover:not(:disabled) { transform: translateY(-2px) scale(1.02); box-shadow: 0 8px 24px rgba(234,88,12,0.44); }
        .pr-save-btn:disabled { opacity: .5; cursor: not-allowed; }
        ...
        .pr-addr-form-wrap {
          margin-bottom: 28px; padding: 24px; background: rgba(255,255,255,0.04); backdrop-filter: blur(16px);
          border: 1.5px solid rgba(234,88,12,0.28); border-radius: 18px;
        }
        .pr-addr-form-hdr { display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; }
        .pr-addr-form-title { font-size: 16px; font-weight: 700; color: #f1f5f9; }
        .pr-addr-close {
          width: 32px; height: 32px; border-radius: 50%;
          background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.10);
        ...
        .pr-addr-close:hover { background: rgba(239,68,68,0.15); color: #f87171; }
        .pr-map-wrap { height: 380px; border-radius: 14px; overflow: hidden; border: 1px solid rgba(255,255,255,0.08); margin-bottom: 20px; }
        .pr-radio-row { display: flex; gap: 16px; flex-wrap: wrap; }
        .pr-radio-label { display: flex; align-items: center; gap: 7px; cursor: pointer; font-size: 13.5px; color: rgba(255,255,255,0.60); }
        .pr-grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
        .pr-grid-3 { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 14px; }
        @media (max-width: 640px) { .pr-grid-2 { grid-template-columns: 1fr; } .pr-grid-3 { grid-template-columns: 1fr; } }
        .pr-checkbox-row { display: flex; align-items: center; gap: 8px; font-size: 13.5px; color: rgba(255,255,255,0.55); }
        .pr-addr-btn-row { display: flex; justify-content: flex-end; gap: 10px; padding-top: 8px; }
        .pr-addr-save-btn {
          display: flex; align-items: center; gap: 7px;
          background: linear-gradient(135deg, #ea580c, #f97316); color: #fff; border: none;
          border-radius: 11px; padding: 10px 22px; font-size: 13.5px; font-weight: 700;
          font-family: 'DM Sans', sans-serif; cursor: pointer; transition: transform .2s, box-shadow .2s;
        }
        .pr-addr-save-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 6px 18px rgba(234,88,12,0.38); }
        .pr-addr-save-btn:disabled { opacity: .5; cursor: not-allowed; }
        .pr-addr-cancel-btn {
        ...
        .pr-addr-card {
          background: rgba(255,255,255,0.04); backdrop-filter: blur(16px); border: 1px solid rgba(255,255,255,0.08);
          border-radius: 16px; padding: 18px 16px; display: flex; align-items: flex-start; gap: 14px;
          transition: border-color .25s, box-shadow .25s, transform .25s;
        }
        .pr-addr-card:hover { border-color: rgba(249,115,22,0.22); transform: translateY(-3px); box-shadow: 0 14px 36px rgba(0,0,0,0.32), 0 0 0 1px rgba(249,115,22,0.12); }
        .pr-addr-body { flex: 1; min-width: 0; }
        .pr-addr-badges { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 8px; }
        .pr-addr-badge { font-size: 10.5px; font-weight: 700; padding: 3px 9px; border-radius: 999px; border: 1px solid transparent; }
        .pr-addr-badge-label { background: rgba(234,88,12,0.14); border-color: rgba(234,88,12,0.25); color: #fb923c; }
        .pr-addr-badge-default { background: rgba(74,222,128,0.12); border-color: rgba(74,222,128,0.22); color: #4ade80; }
        .pr-addr-name { font-size: 15px; font-weight: 700; color: #f1f5f9; margin-bottom: 2px; }
        .pr-addr-phone { font-size: 12.5px; color: rgba(255,255,255,0.38); margin-bottom: 7px; }
        ...
        .pr-actions-section {
          margin-top: 28px; background: rgba(255,255,255,0.04); backdrop-filter: blur(16px);
        ...
        .pr-action-item.danger:hover { background: rgba(239,68,68,0.06); }
        .pr-action-left { display: flex; align-items: center; gap: 12px; }
        .pr-action-ico { width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        .pr-action-title { font-size: 14px; font-weight: 600; color: #f1f5f9; }
        .pr-action-sub { font-size: 12px; color: rgba(255,255,255,0.35); margin-top: 1px; }
        .pr-action-item.danger .pr-action-title { color: #f87171; }
        .pr-action-arrow { color: rgba(255,255,255,0.22); }
        ...
        @media (max-width: 640px) { .pr-hero-card { gap: 16px; } .pr-stats { gap: 16px; } }

        .prwl-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 16px; }
        .prwl-card {
          background: rgba(255,255,255,0.04); backdrop-filter: blur(16px); border: 1px solid rgba(255,255,255,0.08);
          border-radius: 18px; overflow: hidden; display: flex; flex-direction: column;
          transition: transform .28s, box-shadow .28s, border-color .28s;
        }
        .prwl-card:hover { transform: translateY(-5px); box-shadow: 0 18px 44px rgba(0,0,0,0.35), 0 0 0 1px rgba(249,115,22,0.20); border-color: rgba(249,115,22,0.22); }
        .prwl-img-wrap { position: relative; height: 180px; overflow: hidden; background: #0f172a; flex-shrink: 0; }
        .prwl-img { width: 100%; height: 100%; object-fit: cover; transition: transform .5s ease; }
        .prwl-card:hover .prwl-img { transform: scale(1.07); }
        .prwl-img-grad { position: absolute; bottom: 0; left: 0; right: 0; height: 60px; background: linear-gradient(to top, rgba(0,0,0,0.65), transparent); pointer-events: none; }
        .prwl-remove {
          position: absolute; top: 9px; right: 9px; z-index: 3;
          width: 30px; height: 30px; border-radius: 50%;
          background: rgba(10,20,40,0.80); backdrop-filter: blur(6px);
        ...
        .prwl-remove:hover { background: rgba(239,68,68,0.20); border-color: rgba(239,68,68,0.35); color: #f87171; transform: scale(1.12); }
        .prwl-info { padding: 12px 14px 14px; display: flex; flex-direction: column; gap: 6px; flex: 1; }
        .prwl-cat { font-size: 10px; font-weight: 700; color: #f97316; text-transform: uppercase; letter-spacing: .12em; }
        .prwl-name { font-size: 13.5px; font-weight: 700; color: #f1f5f9; line-height: 1.35; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
        .prwl-price { font-size: 16px; font-weight: 800; color: #f97316; text-shadow: 0 0 14px rgba(249,115,22,0.28); }
        .prwl-cart-btn {
        ...
        .prwl-cart-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 6px 18px rgba(234,88,12,0.38); }
        .prwl-cart-btn.oos { background: rgba(255,255,255,0.07); color: rgba(255,255,255,0.32); box-shadow: none; cursor: not-allowed; }
        .prwl-loader { display: flex; align-items: center; justify-content: center; padding: 60px 20px; gap: 12px; font-size: 14px; color: rgba(255,255,255,0.40); }
        ...
        .pr-order-card:last-child { margin-bottom: 0; }
        .pr-order-card:hover { border-color: rgba(249,115,22,0.28); box-shadow: 0 14px 36px rgba(0,0,0,0.32), 0 0 0 1px rgba(249,115,22,0.10); transform: translateY(-3px); }
        .pr-order-hdr { display: flex; align-items: center; justify-content: space-between; padding: 16px 18px; flex-wrap: wrap; gap: 10px; border-bottom: 1px solid rgba(255,255,255,0.06); background: rgba(255,255,255,0.02); }
        .pr-order-id { font-family: monospace; font-size: 12px; font-weight: 700; color: rgba(255,255,255,0.50); background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.10); padding: 3px 10px; border-radius: 7px; }
        .pr-order-date { font-size: 12px; color: rgba(255,255,255,0.35); }
        .pr-order-status { display: inline-flex; align-items: center; gap: 5px; font-size: 11px; font-weight: 800; padding: 4px 11px; border-radius: 999px; white-space: nowrap; }
        .pr-order-status-dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; }
        .pr-order-body { padding: 16px 18px; }
        .pr-order-items { display: flex; flex-direction: column; gap: 10px; margin-bottom: 14px; }
        .pr-order-item { display: flex; align-items: center; gap: 12px; }
        .pr-order-item-img { width: 54px; height: 54px; border-radius: 10px; object-fit: cover; flex-shrink: 0; background: #0f172a; border: 1px solid rgba(255,255,255,0.07); }
        .pr-order-item-name { font-size: 13.5px; font-weight: 600; color: #f1f5f9; margin-bottom: 2px; }
        .pr-order-item-qty  { font-size: 12px; color: rgba(255,255,255,0.38); }
        .pr-order-item-price { margin-left: auto; flex-shrink: 0; font-size: 14px; font-weight: 800; color: #f97316; }
        .pr-order-footer { display: flex; align-items: center; justify-content: space-between; padding-top: 12px; border-top: 1px solid rgba(255,255,255,0.06); flex-wrap: wrap; gap: 8px; }
        .pr-order-total-lbl { font-size: 12px; color: rgba(255,255,255,0.35); }
        .pr-order-total-val { font-size: 18px; font-weight: 800; background: linear-gradient(90deg,#fb923c,#f97316); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .pr-order-track-btn { display: inline-flex; align-items: center; gap: 6px; background: rgba(249,115,22,0.10); border: 1px solid rgba(249,115,22,0.22); color: #fb923c; font-size: 12.5px; font-weight: 700; padding: 7px 14px; border-radius: 9px; text-decoration: none; transition: background .2s, box-shadow .2s; }
        .pr-order-track-btn:hover { background: rgba(249,115,22,0.18); box-shadow: 0 4px 14px rgba(249,115,22,0.22); }
        .pr-orders-loader { display: flex; align-items: center; justify-content: center; padding: 60px 20px; gap: 12px; font-size: 14px; color: rgba(255,255,255,0.38); }
        .pr-spin { width: 22px; height: 22px; border-radius: 50%; border: 2.5px solid rgba(249,115,22,0.20); border-top-color: #f97316; animation: prSpin .75s linear infinite; flex-shrink: 0; }
        @keyframes prSpin { to { transform: rotate(360deg); } }

        @keyframes cpwOverlayIn { from{opacity:0} to{opacity:1} }
        @keyframes cpwModalIn { from{opacity:0;transform:scale(0.90) translateY(-16px)} to{opacity:1;transform:scale(1) translateY(0)} }
        @keyframes cpwSpin { to{transform:rotate(360deg)} }
      `}</style>

      {/* ── Modals rendered via Portal at root level — outside all stacking contexts ── */}

      {showPasswordModal && (
        <ChangePasswordModal onClose={() => setShowPasswordModal(false)} />
      )}

      {/* ── ADDED: Delete Account Modal via React Portal ── */}
        ...
        >
          <div
            ...
            style={{
              ...
              position: 'relative', overflow: 'hidden',
              boxShadow: '0 0 0 1px rgba(239,68,68,0.12), 0 40px 100px rgba(0,0,0,0.75)',
              animation: 'cpwModalIn 0.28s cubic-bezier(0.34,1.56,0.64,1) both',
            }}
          >
            {/* Top red line */}
            <div style={{ position:'absolute', top:0, left:0, right:0, height:2, background:'linear-gradient(90deg,#dc2626,#ef4444,transparent)', borderRadius:'22px 22px 0 0' }} />

            {/* Warning Icon */}
            <div style={{ width:58, height:58, borderRadius:16, background:'rgba(239,68,68,0.12)', border:'1px solid rgba(239,68,68,0.28)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 18px', boxShadow:'0 0 24px rgba(239,68,68,0.15)' }}>
              <FiAlertTriangle size={28} color="#f87171" />
            </div>

            {/* Title */}
            <p style={{ fontFamily:'Playfair Display,serif', fontSize:22, fontWeight:800, color:'#fff', textAlign:'center', marginBottom:10 }}>
              Delete Account
            </p>
            <p style={{ fontSize:14, color:'rgba(255,255,255,0.45)', textAlign:'center', lineHeight:1.65, marginBottom:22 }}>
              This will permanently delete your account and all your data including addresses and wishlist.{' '}
              <strong style={{ color:'#f87171' }}>This action cannot be undone.</strong>
            </p>

            {/* Error */}
            {deleteError && (
              <div style={{ background:'rgba(239,68,68,0.09)', border:'1px solid rgba(239,68,68,0.24)', borderRadius:10, padding:'11px 14px', marginBottom:18, fontSize:13.5, color:'#fca5a5', textAlign:'center' }}>
                {deleteError}
              </div>
            )}
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              <button
                ...
                style={{
                  ...
                  background: deleteLoading ? 'rgba(239,68,68,0.40)' : 'linear-gradient(135deg,#dc2626,#ef4444)',
                  color:'#fff', fontSize:14.5, fontWeight:700,
                  fontFamily:'DM Sans,sans-serif',
                  cursor: deleteLoading ? 'not-allowed' : 'pointer',
                  display:'flex', alignItems:'center', justifyContent:'center', gap:8,
                  boxShadow: deleteLoading ? 'none' : '0 5px 18px rgba(220,38,38,0.35)',
                  transition:'transform 0.2s, box-shadow 0.2s',
                }}
              >
                {deleteLoading ? (
                  <>
                    <svg style={{ animation:'cpwSpin 0.7s linear infinite', flexShrink:0 }} width="16" height="16" viewBox="0 0 24 24" fill="none">
                      <circle cx="12" cy="12" r="10" stroke="white" strokeWidth="4" strokeOpacity=".25"/>
                      <path fill="white" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
                    </svg>
                    Deleting Account...
                  </>
                ) : (
                  <><FiTrash2 size={15} /> Yes, Delete My Account</>
                )}
              </button>
          ...
          <div className="pr-card">
            ...
            {activeTab === 'profile' && (
              <div>
                ...
                <form onSubmit={handleSubmit}>
                  <div className="pr-field">
                    <label className="pr-label"><FiUser size={12} /> Full Name</label>
                    <input type="text" value={formData.fullName} disabled={!editMode} className="pr-input"
                      onChange={(e) => { setFormData({ ...formData, fullName: e.target.value }); setErrors({ ...errors, fullName: '' }); }} />
                    {errors.fullName && <p className="pr-field-err">{errors.fullName}</p>}
                  </div>
                  <div className="pr-field">
                    <label className="pr-label"><FiMail size={12} /> Email Address</label>
                    <input type="email" value={formData.email} disabled={!editMode} className="pr-input"
                      onChange={(e) => { setFormData({ ...formData, email: e.target.value }); setErrors({ ...errors, email: '' }); }} />
                    {errors.email && <p className="pr-field-err">{errors.email}</p>}
                  </div>
                  <div className="pr-field">
                    <label className="pr-label"><FiPhone size={12} /> Phone Number</label>
                    <input type="tel" value={formData.phone} disabled={!editMode} placeholder="9876543210" className="pr-input"
                      onChange={(e) => { setFormData({ ...formData, phone: e.target.value }); setErrors({ ...errors, phone: '' }); }} />
                    {errors.phone && <p className="pr-field-err">{errors.phone}</p>}
                  </div>
                  {editMode && (
                    <div className="pr-btn-row">
                      ...
                      <button type="button" className="pr-cancel-btn"
                        onClick={() => { setEditMode(false); setErrors({}); if (user) setFormData({ fullName: user.fullName || '', email: user.email || '', phone: user.phone || '' }); }}>
                        <FiX size={15} /> Cancel
                      </button>
                    </div>
                  )}
                  ...
                  <div className="pr-actions-section">
                    ...
                    >
                      <div className="pr-action-left">
                        ...
                        <div>
                          <p className="pr-action-title">Change Password</p>
                          <p className="pr-action-sub">{user?.googleId ? 'Managed by Google' : 'Update your account password'}</p>
                        </div>
                      </div>
                      <FiChevronRight size={16} className="pr-action-arrow" />
                    </div>
                    ...
                    {/* ── UPDATED: Delete Account — now opens modal ── */}
                    <div className="pr-action-item danger" onClick={() => setShowDeleteModal(true)}>
                      <div className="pr-action-left">
                        <div className="pr-action-ico" style={{ background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.18)' }}>
                          <FiAlertTriangle size={16} color="#f87171" />
                        </div>
            ...
            {activeTab === 'orders' && (
              <div>
                <div className="pr-sec-title">Order History</div>
                {ordersLoading ? (
                  <div className="pr-orders-loader"><div className="pr-spin" /> Loading your orders...</div>
                ) : orders.length === 0 ? (
                  <div className="pr-empty">
                    <div className="pr-empty-icon"><FiPackage size={30} color="#fb923c" /></div>
                    <p className="pr-empty-title">No orders yet</p>
                      ...
                      return (
                        <div key={order._id} className="pr-order-card">
                          <div className="pr-order-hdr">
                            ...
                            <span className="pr-order-status" style={{ background:sc.bg, border:`1px solid ${sc.border}`, color:sc.color }}>
                              <span className="pr-order-status-dot" style={{ background:sc.color }} />
                              {order.orderStatus?.charAt(0).toUpperCase() + order.orderStatus?.slice(1)}
                            </span>
                          </div>
                          <div className="pr-order-body">
                            <div className="pr-order-items">
                              {(order.orderItems || []).slice(0, 3).map((item, idx) => (
                                <div key={idx} className="pr-order-item">
                                  <img src={getImageUrl(item.image) || 'https://via.placeholder.com/54x54?text=Product'} alt={item.name} className="pr-order-item-img" onError={e => { e.target.src = 'https://via.placeholder.com/54x54?text=Product'; }} />
                                  <div style={{ flex:1, minWidth:0 }}>
                                    <p className="pr-order-item-name">{item.name}</p>
                                    <p className="pr-order-item-qty">Qty: {item.quantity}</p>
                                  </div>
            ...
            {activeTab === 'wishlist' && (
              <div>
                ...
                {wishlistLoading ? (
                  <div className="prwl-loader"><div className="pr-spin" /> Loading your wishlist...</div>
                ) : contextWishlistItems.length === 0 ? (
                  <div className="pr-empty">
                    <div className="pr-empty-icon"><FiHeart size={30} color="#fb923c" /></div>
                    <p className="pr-empty-title">Your wishlist is empty</p>
                  ...
                  <div className="prwl-grid">
                    {contextWishlistItems.map((item) => {
                      ...
                      return (
                        <div key={product._id} className="prwl-card">
                          <div className="prwl-img-wrap">
                            <img src={getImageUrl(product.images?.[0])} alt={product.name} className="prwl-img" onError={e => { e.target.src = 'https://via.placeholder.com/220x180?text=Product'; }} />
                            <div className="prwl-img-grad" />
                            <button className="prwl-remove" onClick={() => handleRemoveFromWishlist(product._id)} title="Remove from wishlist"><FiTrash2 size={13} /></button>
                          </div>
                          <div className="prwl-info">
                            <p className="prwl-cat">{product.category?.name || 'Handloom'}</p>
                            <p className="prwl-name">{product.name}</p>
                            <p className="prwl-price">{formatPrice(product.price)}</p>
                            <Link to={`/products/${product._id}`} style={{ fontSize:12, color:'rgba(255,255,255,0.38)', textDecoration:'none', marginBottom:4 }}>View Details →</Link>
                            <button className={`prwl-cart-btn ${product.stock === 0 ? 'oos' : ''}`} disabled={product.stock === 0} onClick={() => { addToCart(product, 1); }}>
                              <FiShoppingCart size={13} />
                              {product.stock === 0 ? 'Out of Stock' : 'Add to Cart'}
                            </button>
                          </div>
